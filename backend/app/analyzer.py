...
def analyze_non_respond_linestate(
    input_paths: list[str],
    output_path: str,
    chunk_size: int = 500000,
    progress_callback = None,
    ...
    """
    Analyzes CDR files for non_respond category and classifies line state.
    """
    if progress_callback:
        progress_callback(0, "scanning", "Iniciando análisis de LineState...")

    total_rows = 0
    invalid_rows = 0
    
    # We'll use a dictionary to accumulate stats per e164
    # e164 -> {count, total_secs}
    stats = {}
    
    # Pass 1: Aggregate metrics
    for input_path in input_paths:
        # We need to know total rows for progress if possible, or just process
        for chunk in pd.read_csv(
            input_path, 
            sep=';', 
            usecols=['e164', 'sip_code', 'tot_secs'], 
            dtype={'e164': str, 'sip_code': float, 'tot_secs': float},
            chunksize=chunk_size,
            on_bad_lines='warn',
            engine='c'
        ):
            ...
            # Filter for non_respond (404, 480)
            # Assuming non_respond is defined by these SIP codes
            chunk = chunk[chunk['sip_code'].isin([404, 480])]
            
            # Drop invalid tot_secs
            chunk = chunk.dropna(subset=['e164', 'tot_secs'])
            
            if chunk.empty:
                continue
                
            # Group by e164
            chunk_stats = chunk.groupby('e164')['tot_secs'].agg(['count', 'sum'])
            
            for e164, row in chunk_stats.iterrows():
                if e164 not in stats:
                    stats[e164] = {'count': 0, 'sum': 0}
                stats[e164]['count'] += row['count']
                stats[e164]['sum'] += row['sum']
            
            if progress_callback:
                # Approximate progress
                progress_callback(50, "processing", f"Procesando registros... {total_rows} filas escaneadas")

    # Pass 2: Classify and Finalize
    if progress_callback:
        progress_callback(90, "finalizing", "Clasificando estados de línea y generando resultados...")
    ...
    for e164, data in stats.items():
        count = data['count']
        total_secs = data['sum']
        avg_secs = total_secs / count if count > 0 else 0
        
        # Classification:
        # 0 a 4 segundos → Inactiva
        # 5 a 9 segundos → Indeterminada
        # 10 o más → Activa
        if avg_secs < 5:
            state = "Inactiva"
            inactiva_count += 1
        ...
        results.append({
            'e164': e164,
            'total_intentos': int(count),
            'suma_tot_secs': round(total_secs, 2),
            'promedio_tot_secs': round(avg_secs, 2),
            'LineState': state
        })
    ...
    summary = {
        'total_numeros_analizados': total_analizados,
        'inactiva_count': inactiva_count,
        'indeterminada_count': indeterminada_count,
        'activa_count': activa_count,
        'inactiva_pct': round((inactiva_count / total_analizados * 100), 2) if total_analizados > 0 else 0,
        'indeterminada_pct': round((indeterminada_count / total_analizados * 100), 2) if total_analizados > 0 else 0,
        'activa_pct': round((activa_count / total_analizados * 100), 2) if total_analizados > 0 else 0,
        'filas_invalidas_descartadas': invalid_rows # Placeholder
    }

    if progress_callback:
        progress_callback(100, "completed", "Análisis de LineState completado.")
